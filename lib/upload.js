...
glob(PUBLIC_ASSETS_SOURCE_DIRECTORY + '/**/*.*', {}, function(error, files) {
    ...
    async.eachLimit(files, 16, function(file, callback) {
        ...
        const key = path.join(STATIC_PATH, file.replace(PUBLIC_ASSETS_SOURCE_DIRECTORY, ''))
        ...
        s3.upload({
          ...
          const cdn2Key = path.join('assets', key)
          // console.log('uploading to s3 fs-cdn2-origin', cdn2Key, file)
          s3.upload({
            ACL: 'public-read',
            Key: cdn2Key,
            Body: fs.createReadStream(file),
