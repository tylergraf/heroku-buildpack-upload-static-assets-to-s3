...
try {
  ...
  AWS.config.accessKeyId = process.env.AWS_ACCESS_KEY_ID || process.env.S3_ACCESS_KEY;
  AWS.config.secretAccessKey = process.env.AWS_SECRET_ACCESS_KEY || process.env.S3_SECRET_ACCESS_KEY;
  AWS.config.region = process.env.AWS_DEFAULT_REGION || 'us-east-1';

  // bucket where static assets are uploaded to
  var AWS_STATIC_BUCKET_NAME = process.env.AWS_STATIC_BUCKET_NAME || process.env.S3_BUCKET_NAME;

  console.log('process.env', process.env);
  console.log('process.env.S3_BUCKET_NAME', process.env.S3_BUCKET_NAME);
  console.log('AWS_STATIC_BUCKET_NAME', AWS_STATIC_BUCKET_NAME);
  // the source directory of static assets
  var AWS_STATIC_SOURCE_DIRECTORY = process.env.AWS_STATIC_SOURCE_DIRECTORY || 'build/static';
  // the prefix assigned to the path, can be used to configure routing rules in CDNs
  var AWS_STATIC_PREFIX = process.env.AWS_STATIC_PREFIX || 'static';
